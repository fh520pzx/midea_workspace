...
def temp_differ(diff):
    ...
    frames = []
    id_tset_time = []
    df = pd.read_csv(r'D:\MyData\fanghui3\Desktop\data\hf_analysis_data.txt',delim_whitespace=True)
    ...
    for id in list_id:
        ...
        appliance_id,tempset, runstatus, modeset, windspeedset, adt, tin, tout, humidity, flag = [],[],[],[],[],[],[],[],[],[]
        list_flag = []
        index = []
        index_new = []
        all_index = []
    ...
    # return result
    # result.to_excel('no.xlsx',index=False)
